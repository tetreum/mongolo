...
class User extends Controller
{
    ...
    public function doLogin ()
    {
        ...
        if (isset($settings["local_auth"])) {
            if (!isset($settings["local_auth"]["users"][$user]) || $settings["local_auth"]["users"][$user] != md5($settings["local_auth"]["salt"] . $password)) {
                throw new AppException(AppException::INVALID_LOGIN);
        }
            $canConnect = $this->mongo->canConnect();
        } else {
            $canConnect = $this->mongo->canConnect($user, $password);
        }

mongolo.navigation = function ()
{
    ...
    var browse = function (path)
    {
        ...
        mongolo.navigation.get(path, function (html) {
    ...
    var parseError = function (data, callback)
    {
        switch (data.error)
        {
            case 11: // unauthorized request (probably session expired)
                mongolo.navigation.redirect("logout?redirect=" + encodeURIComponent(location.pathname + location.search));
                break;
